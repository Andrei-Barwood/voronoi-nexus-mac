...
#
# batch-process.sh
# Advanced example: Process multiple malware sample files with delaunay-sentinel
#
# Usage:
...
if [ "$USE_DEBUG" = true ]; then
    BINARY="$PROJECT_ROOT/.build/debug/delaunay-sentinel"
else
    if [ -f "$PROJECT_ROOT/.build/release/delaunay-sentinel" ]; then
        BINARY="$PROJECT_ROOT/.build/release/delaunay-sentinel"
    else
        BINARY="$PROJECT_ROOT/.build/debug/delaunay-sentinel"
    fi
fi
...
echo "🚀 Batch Processing with delaunay-sentinel"
echo "Binary: $BINARY"
